...
> [!TIP]
> **OpenAPI Swagger UI**: The application provides an interactive API documentation portal generated by FastAPI. Visit `/docs` on the running server to see the fully annotated interface with structured metadata (`tags_metadata`), endpoint summaries, descriptions, and strongly-typed `Pydantic` schema definitions.
